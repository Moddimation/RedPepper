...
void alLiveActorFunction::calcAnimDirect(al::LiveActor* actor) {
    ...
    if (actor->mModelKeeper)
        al::setBaseMtxAndCalcAnim(actor, baseMtx,
                                  actor->mActorPoseKeeper->getScale() /* al::getScale */);
...
void initNerveAction(LiveActor* actor, const char* name,
                     alNerveFunction::NerveActionCollector* collector, int maxNerveStates) {
